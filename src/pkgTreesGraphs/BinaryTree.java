...
public class BinaryTree {
	...
	//For Recovering BST problem
	TreeNode firstElem = null, secElem = null, prevElem = new TreeNode(Integer.MIN_VALUE);
	...
	public static void main(String args[]){
		...
		btree.createLevelLinkedList(root);
//		TreeNode node = root.leftChild.rightChild;
//		btree.printCousins(root, node);
...
//		
//		System.out.println("Get path Sum:");
//		btree.getPathSum(root, 42);
		
//		System.out.println("Level Order Zigzag:");
//		btree.levelOrderZigZag(root);
		
//		System.out.println("After Swapping:");
//		btree.recoverBST(root);
//		btree.inOrderIter(root);
...
//			nodeData = node.data;
	...
	public void levelOrderZigZag(TreeNode root){
		...
		while(!cq.isEmpty()){
			...
			if(node.leftChild != null){
				nq.offer(node.leftChild);
			}
				
			if(node.rightChild != null){
				nq.offer(node.rightChild);
			}			
			cl.add(node.data);
			if(cq.isEmpty()){
				cq = nq;
				...
				leftRight = !leftRight;
				result.add(new ArrayList<Integer>(cl));
				cl.clear();
			}
		}
...
//			System.out.println();
	 ...
	 * =========================================================
	 */
	void createLevelLinkedList(TreeNode root, ArrayList<LinkedList<Integer>> lists, int level){
		if(root == null)
			return;
    ...
    public void recoverBST(TreeNode root){
    	...
    	if(firstElem != null && secElem != null){
    		int temp = firstElem.data;
    		firstElem.data = secElem.data;
    		secElem.data = temp;
    	}
    }
    ...
    public void traverse(TreeNode root){
    	...
    	if(firstElem == null && prevElem.data >= root.data)
    		firstElem = prevElem;
    	if(firstElem != null && prevElem.data >= root.data)
    		secElem = root;
    	prevElem = root;
    ...
    public void getPathSum(TreeNode root, int target){
    	List<List<Integer>> result = new ArrayList<List<Integer>>();
    	if(root == null)
    		return;
    	getPathSum(root, 0, target, new ArrayList<Integer>(), result);
    	for(List<Integer> list : result){
    		System.out.println(list);
    	}
    }
    
    public void getPathSum(TreeNode root, int sum, int target, List<Integer> curr, List<List<Integer>> result){
    	if(root == null)
    		return;
    	...
    	if(sum+root.data == target && root.leftChild == null && root.rightChild == null){
    	...
    	if(root.leftChild != null){
    		getPathSum(root.leftChild, sum+root.data, target, curr, result);
//        	sum -= root.data;
//        	curr.remove(curr.size()-1);
    	}
    	if(root.rightChild != null){
    		getPathSum(root.rightChild, sum+root.data, target, curr, result);
    ...
    public int getLevel(TreeNode root, TreeNode node, int level){
    	if(root == null)
    		return 0;
    	if(root == node)
    		return level;
    	
    	int left = getLevel(root.leftChild, node, level+1);
    	if(left == 0 )
    ...
    public void printGivenLevel(TreeNode root, TreeNode node, int level){
    	...
    	if(level == 2){
    		...
    		printGivenLevel(root.leftChild, node, level-1);
    		printGivenLevel(root.rightChild, node, level-1);
    	}
    }
